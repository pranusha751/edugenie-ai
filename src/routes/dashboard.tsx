    ...
    <MobileShell>
      <header className="mb-5 flex items-center justify-between">
        ...
        <Link to="/profile" className="relative flex h-11 w-11 items-center justify-center rounded-2xl gradient-primary text-xl shadow-glow">
          🧑‍🎓
          <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-destructive text-[9px] font-bold text-destructive-foreground">3</span>
        </Link>
      </header>
      ...
      <div className="mb-2 flex items-center justify-between">
        ...
        <Link to="/progress" className="text-xs text-primary font-medium">See all</Link>
      </div>
      <div className="space-y-2">
        {subjects.slice(0, 3).map((s) => (
          <div key={s.name} className="glass flex items-center gap-3 rounded-2xl p-3">
            <div className={`flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br ${s.color} text-xl shadow-soft`}>
              {s.icon}
            </div>
            <div className="flex-1">
...
function QuickAction({ to, icon: Icon, label, sub, gradient }: any) {
  return (
    <Link to={to} className="glass group relative overflow-hidden rounded-2xl p-4 transition hover:scale-[1.02]">
      <div className={`mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br ${gradient} shadow-soft`}>
